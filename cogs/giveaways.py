...
class Giveaways(commands.Cog):
    ...
    async def db_get(self, giveaway_id: str) -> Optional[dict]:
        d = await db.fetch_one("SELECT * FROM giveaways WHERE id = ?", [giveaway_id])
        if not d:
            return None
    ...
    async def db_all_active(self) -> list[dict]:
        rows = await db.fetch_all("SELECT * FROM giveaways WHERE status = 'active'")
        result = []
        for d in rows:
            d["entries"] = json.loads(d["entries"])
            d["winners"] = json.loads(d["winners"])
    ...
    async def db_update(self, giveaway_id: str, fields: dict):
        ...
        await db.execute(f"UPDATE giveaways SET {set_clause} WHERE id = ?", values)
    ...
    async def giveaway_delete(self, interaction: discord.Interaction, giveaway_id: str):
        ...
        await db.execute("DELETE FROM giveaways WHERE id = ?", [giveaway_id.upper()])
